classdef trc_file < handle
    ...
    methods
        ...
        function self=trc_file(f_name)
            ...
            self.a_fd=fopen(self.a_file_name,'rb');
        ...
        function [cdata]=combine_data(self,data)
            ...
            for i=1:length(self.a_channs_cell)
                ...
                sig1_idx=find(strcmpi(self.a_file_elec_cell,sig1));
                if isempty(sig1_idx)
                    cdata=[];
                else
                    if ~isempty(sig2)
        ...
        function [data,time]=def_data_access(self, wsize, step, channs_cell, offset)
            ...
            fseek(self.a_fd,self.a_curr_bytes_offset,'bof')
            ...
            try%Try if it is possible to acquire the requested amount of data
                ...
                data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],self.a_dtype)';
        ...
        %%
        function num=has_next_window(self)
            %==============================================================
            %"has_next_window" method
            %This method returns whether a next data portion is available.
            
            curPos=ftell(self.a_fd);
            fseek(self.a_fd,0,'eof');
            self.a_abs_file_pt=ftell(self.a_fd);
            fseek(self.a_fd,curPos,'bof');
            window_size_bytes=self.a_n_chan*self.a_wsize*self.a_samp_freq*self.a_n_bytes;
            num=floor((self.a_abs_file_pt-self.a_last_bytes_offset)/window_size_bytes);
            disp(['has_next_window: ' num2str(num)])
        end
        ...
        function data=get_next_window(self)
            ...
            if self.a_last_bytes_offset+window_size_bytes<=self.a_abs_file_pt
                self.a_last_flagg=0;
...
%ab%                    if(self.a_n_bytes~=2)
                    if(self.a_n_bytes==4)
                        data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],self.a_dtype)';
                    elseif(self.a_n_bytes==2)
                        data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],self.a_dtype)';
                    else
                        error('Wrong data format');
                    end
%ab%                    data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],'uint16')';
 %                   data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],'float')';
                    %data=fread(self.a_fd,[self.a_n_chan self.a_wsize*self.a_samp_freq],self.a_dtype)';
                    %data=data(:,self.a_channs_cell);
                    self.a_is_last_segment=0;
                    self.a_is_first_segment=0;
                    ...
                    error('Could not read the requested amount of data from TRC file')
                    self.a_data_map.Format=self.a_dtype;%Consider independent data samples, not arranged acourding to the number of channels
                    self.a_data_map.Repeat=Inf;%Repeat the data format to the end of file
                    last_data=self.a_data_map.Data;%Get data
                    n_last_samp=size(last_data,1);%Compute how many individual samples were readed
                ...
                self.a_last_data_mat=data;
                disp('got_next_window successfull')
            else
                ...
                disp('got_next_window not successfull')
            end
